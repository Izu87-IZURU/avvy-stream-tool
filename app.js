...
async function ensureOfficialGifts(userId){
  ...
  const existingOfficial=(existingAll||[])
...
async function load(){
  ...
  if(user){
    ...
    const {
      data:appUser,
      error:appUserError
    }=await sb.rpc('get_or_create_app_user');
    ...
    */
    ...
    state.giftCounts=Object.fromEntries(
      (gc.data||[]).map(x=>[
        x.gift_id,
        x.count
      ])
    ...
    state.itemCounts=Object.fromEntries(
      (ic.data||[]).map(x=>[
    ...
    state.eventSettings=Object.fromEntries(
      (es.data||[]).map(x=>[
        x.event_key,
        x.enabled
      ])
    ...
    state.eventGiftSettings=Object.fromEntries(
      (egs.data||[]).map(x=>[
        x.gift_id,
        x.enabled
      ])
    ...
    state.giftTargets=Object.fromEntries(
      (gts.data||[]).map(x=>[
...
function giftTypeSummary(gs){
  ...
  const achieved=
    gs.filter(
      g=>Number(state.giftCounts[g.id]||0)>0
    ).length;

  const remaining=
    Math.max(0,total-achieved);
          ...
          g=>
            `
              ...
              >
                ...
                <span>
                  ${Number(state.giftCounts[g.id]||0)}個
                </span>
...
function targetKey(
  ...
  return `${source}|${eventKey||''}|${Number(coin)}`;
}
...
function targetValue(
  ...
  return state.giftTargets[
    targetKey(source,eventKey,coin)
  ]||50;
}
...
async function editGiftTarget(raw){
  ...
  const {error}=await sb
    ...
    .upsert(
      ...
      {
        onConflict:'user_id,source,event_key,coin'
      }
    );
    ...
    <div class="grid">
      ...
      <div class="card">
        ...
        <div class="row">
          ...
          <span
            class="pill"
            style="background:${state.accentColor};color:#fff"
          >
            現在のカラー
          </span>
...
async function incrementGift(id){
  ...
  const next=
    Number(state.giftCounts[id]||0)+1;
...
function bind(){
  ...
  */
  ...
  document
    .querySelectorAll('[data-reset-category]')
    .forEach(
      b=>
        b.onclick=()=>
          resetCategory(
  ...
  */
  ...
  document
    .querySelectorAll('[data-reset-source]')
    .forEach(
      b=>
        b.onclick=()=>
          resetCoin(
  ...
  */
  ...
  document
    .querySelectorAll('[data-reset-event]')
    .forEach(
      b=>
        b.onclick=()=>
          resetEvent(
  ...
  */
  ...
  document
    .querySelectorAll('[data-del-origift]')
    .forEach(
      b=>
        b.onclick=async()=>{
  ...
  */
  ...
  document
    .querySelectorAll('[data-open-custom]')
    .forEach(
      b=>
        b.onclick=()=>{
